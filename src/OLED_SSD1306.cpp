...
void OLED_SSD1306::print6x8Str(unsigned char x, unsigned char y, unsigned char ch[])
{
	unsigned char c = 0, i = 0, j = 0;
	while (ch[j] != '\0')
	{
...
void OLED_SSD1306::print8x8Str(unsigned char x, unsigned char y, unsigned char ch[])
{
	unsigned char c = 0, i = 0, j = 0;
	while (ch[j] != '\0')
	{
...
void OLED_SSD1306::drawPixel(char x, char y)
{
	if((x < 0) || (x >= OLED_WIDTH) || (y < 0) || (y >= OLED_HEIGHT)) // Check for boundaries
		...
		this->buffer[x + (y / 8) * OLED_WIDTH] |= _BV((y % 8)); // Store pixel in the buffer
}
...
void OLED_SSD1306::drawRect(char x1, char y1, char x2, char y2)
{
	...
	for(y = y1; y < y2; y++) // Iterate over y range, draw line on x2 
		...
		//drawRect( 0,  0, 50, 50);
	//this->drawLine(x1, y1, x2 - 1, y1); // top
	//this->drawLine(x2, y1, x2, y2 - 1); // right
	//this->drawLine(x2, y2, x1 + 1, y2); // bottom
	//this->drawLine(x1, y2, x1, y1 + 1); // left
}
